...
const TodoList = () => {
  ...
  const addTodo = (e) => {
    ...
    if (todoEditId !== null) {
      ...
      todo.autor = formData.autor
      todo.fechaInicio = formData.fechaInicio
      todo.fechaFinal = formData.fechaFinal
      setTodoArray(newTodo)
      setTodoEditId(null)
      setFormData({ titulo: '', descripcion: '', autor: '', fechaInicio: '', fechaFinal: '' })
    } else {
  ...
  return (
    <div className="container w-75">
      <form className="input-group shadow rounded p-3" onSubmit={addTodo}>
        <input className="form-control" type="text" name="titulo" placeholder="Nombre de la tarea" value={formData.titulo} onChange={handleChange} />
        ...
        <input className="form-control" type="date" name="fechaInicio" placeholder="Fecha de inicio" value={formData.fechaInicio} onChange={handleChange} />
        <input className="form-control" type="date" name="fechaFinal" placeholder="Fecha de fin" value={formData.fechaFinal} onChange={handleChange} />
        <input className="form-control" type="text" name="autor" placeholder="Autor" value={formData.autor} onChange={handleChange} />
      ...
      <Modal isOpen={modalEliminar.isOpen} onClose={() => setModalEliminar({ isOpen: false, todo: {} })}>
        <div className='container text-center py-5'>
          ...
          <div className='w-100 d-flex justify-content-center mt-2'>
            <button className='btn btn-danger mx-1' onClick={() => deleteTodo(modalEliminar.todo.id)}>Si</button>
            <button className='btn btn-success mx-1' onClick={() => setModalEliminar({ isOpen: false, todo: {} })}>NO</button>
          </div>
        </div>
      </Modal>
    </div>
